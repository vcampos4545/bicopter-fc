...
| `EstimatorTask` | 8 | Queue-driven, no independent period (blocks on the sensor queue, ≤10 ms wait to keep feeding the watchdog if `SensorTask` stalls) | Receives each sample and logs/discards it - no estimation math exists yet. | Milestones 7-8: `flight_core` math library + attitude/altitude estimator. |
